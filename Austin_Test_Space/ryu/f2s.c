...
static inline floating_decimal_32 f2d(const uint32_t ieeeMantissa, const uint32_t ieeeExponent) {
  ...
  if (ieeeExponent == 0) {
    ...
    m2 = (1ul << FLOAT_MANTISSA_BITS) | ieeeMantissa;
  }
  ...
  if (e2 >= 0) {
    ...
    if (q != 0 && (vp - 1) / 10 <= vm / 10) {
      ...
      lastRemovedDigit = (uint16_t) (mulPow5InvDivPow2(mv, q - 1, -e2 + (int32_t) q - 1 + l) % 10);
    }
    if (q <= 9) {
    ...
    if (q != 0 && (vp - 1) / 10 <= vm / 10) {
      ...
      lastRemovedDigit = (uint16_t) (mulPow5divPow2(mv, (uint32_t) (i + 1), j) % 10);
    }
    if (q <= 1) {
  ...
  if (vmIsTrailingZeros || vrIsTrailingZeros) {
    ...
    while (vp / 10 > vm / 10) {
      ...
      lastRemovedDigit = (uint16_t) (vr % 10);
      vr /= 10;
      vp /= 10;
      vm /= 10;
      ++removed;
...
int f2s_buffered_n(float f, char* result) {
  ...
  const uint32_t ieeeMantissa = bits & ((1ul << FLOAT_MANTISSA_BITS) - 1);
